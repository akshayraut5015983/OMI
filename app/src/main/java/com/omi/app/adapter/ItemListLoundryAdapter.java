...
public class ItemListLoundryAdapter extends RecyclerView.Adapter<ItemListLoundryAdapter.SubServiceHolder> {
    private List<ItemLoundryInfo> arrCity;
    private Context mContext;
    private LayoutInflater mLayoutInflater;
    private String statusA = "Active", statusB = "Deactive";
    private int num = 0;

    public ItemListLoundryAdapter(List<ItemLoundryInfo> arrCity, Context mContext) {
    ...
    public void onBindViewHolder(@NonNull final SubServiceHolder cityViewHolder, final int position) {
        final ItemLoundryInfo serviceItem = arrCity.get(position);
        cityViewHolder.tvQty.setText(arrCity.get(position)._0.name);
